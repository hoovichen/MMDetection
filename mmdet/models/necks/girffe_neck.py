...
class ConvBnAct2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, padding='', bias=False,
        ...
        self.conv = build_conv_layer(None, in_channels, out_channels, kernel_size, stride=stride, padding=padding,
...
class GiraffeNeck(nn.Module):
    ...
    def __init__(self, min_level, max_level, num_levels, norm_layer, norm_kwargs, act_type, fpn_config, fpn_name,
                 fpn_channels, weight_method, depth_multiplier, width_multiplier, with_backslash, with_slash,
                 with_skip_connect, skip_connect_type, out_fpn_channels, pad_type, downsample_type, upsample_type,
                 apply_resample_bn, conv_after_downsample, redundant_bias, fpn_cell_repeats, separable_conv,
                 conv_bn_relu_pattern, feature_info, alternate_init):
        ...
        fpn_config = fpn_config or get_fpn_config(
            fpn_name, min_level=min_level, max_level=max_level, weight_method=weight_method,
            depth_multiplier=neck_depth, with_backslash=with_backslash, with_slash=with_slash,
            with_skip_connect=with_skip_connect, skip_connect_type=skip_connect_type)
