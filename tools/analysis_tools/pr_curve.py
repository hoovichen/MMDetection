...
import pandas as pd
import mmengine
from mmengine.evaluator import metric
...
def parse_args():
    ...
    parser.add_argument('pkl_result', help='pkl result file path')
...
def main():
    ...
    pkl_results = mmengine.load(args.pkl_result)
    # convert pkl file (list[list | tuple | ndarray]) to json
    print(pkl_results)
    json_results, _ = dataset(pkl_results)
